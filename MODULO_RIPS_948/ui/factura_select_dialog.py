...
class FacturaSelectDialog(QDialog):
    """Lista archivos XML/JSON de factura (excluye JSON RIPS por nombre)."""

    def __init__(self, folder: Path, parent=None) -> None:
        super().__init__(parent)
        self.setWindowTitle("Seleccionar archivos de factura (XML / JSON)")
        self.resize(560, 420)
        self._paths: list[Path] = []
        for pattern in ("*.xml", "*.json"):
            for path in sorted(folder.glob(pattern)):
                if path.name.lower().startswith("rips_"):
                    continue
        ...
        layout.addWidget(
            QLabel(
                f"Carpeta: {folder}\n"
                "Seleccione XML o JSON de factura electrónica (no RIPS):"
            )
        )
        ...
        if not self._paths:
            layout.addWidget(QLabel("No hay XML ni JSON de factura en esta carpeta."))
